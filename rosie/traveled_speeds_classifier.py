...
class TraveledSpeedsClassifier(TransformerMixin):
    ...
    def fit(self, X):
        self._X = self.__aggregate_dataset(X)
        self.polynomial = np.polyfit(self._X['expenses'].astype(np.long),
                                     self._X['distance_traveled'].astype(np.long),
                                     3)
        self._polynomial_fn = np.poly1d(self.polynomial)
        self._X = self.__classify_dataset(self._X)
        return self

    def predict(self, X):
        check_is_fitted(self, 'polynomial')
        ...
        is_outlier = \
            (_X['subquota_description'] == 'Congressperson meal') & \
            (_X['expenses_threshold_outlier'] | \
             _X['traveled_speed_outlier'])
        y = is_outlier.astype(np.int).replace({1: -1, 0: 1})
        return y

    def __aggregate_dataset(self, X):
        _X = X.query("subquota_description == 'Congressperson meal'")
        _X = _X.groupby(self.AGG_KEYS) \
        ...
        _X = pd.merge(_X, expenses, left_on=self.AGG_KEYS, right_on=self.AGG_KEYS)
        return _X
