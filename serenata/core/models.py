from django.db import models


class Document(models.Model):

    document_id = models.IntegerField('Document ID')
    congressperson_name = models.CharField('Congressperson name', max_length=64)
    congressperson_id = models.IntegerField('Congressperson ID')
    congressperson_document = models.IntegerField('Congressperson document')
    term = models.IntegerField('Term')
    state = models.CharField('State', max_length=2)
    party = models.CharField('Party', max_length=16)
    term_id = models.IntegerField('Term ID')
    subquota_number = models.IntegerField('Subquote ID')
    subquota_description = models.CharField('Subquota descrition', max_length=64)
    subquota_group_id = models.IntegerField('Subquota group ID')
    subquota_group_description = models.CharField('Subquota group description', max_length=64)
    supplier = models.CharField('Supplier', max_length=128)
    cnpj_cpf = models.CharField('CNPJ or CPF', max_length=14)
    document_number = models.IntegerField('Document number')
    document_type = models.IntegerField('Document type')
    issue_date = models.DateTimeField('Issue date')
    document_value = models.DecimalField('Document value', max_digits=10, decimal_places=3)
    remark_value = models.DecimalField('Remark value', max_digits=10, decimal_places=3)
    net_value = models.DecimalField('Net value', max_digits=10, decimal_places=3)
    month = models.IntegerField('Month')
    year = models.IntegerField('Year')
    installment = models.IntegerField('Installment')
    passenger = models.CharField('Passenger', max_length=64)
    leg_of_the_trip = models.IntegerField('Leg of the trip')
    batch_number = models.IntegerField('Batch number')
    reimbursement_number = models.IntegerField('Reimbursement number')
    reimbursement_value = models.DecimalField('Reimbusrsement value', max_digits=10, decimal_places=3)
    applicant_id = models.IntegerField('Applicant ID')
